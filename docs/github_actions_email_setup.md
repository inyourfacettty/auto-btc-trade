...
意思是 UTC 时间每 4 小时的第 1 分钟执行一次。换成北京时间大约是：
...
邮件正文里的更新时间和 K 线时间都按北京时间展示。

GitHub-hosted Actions 的定时任务可能延迟几分钟，极端情况下会更久；如果你要求严格在 4H 收盘后 1-2 分钟收到邮件，建议改成 self-hosted runner 跑在本机、NAS 或 VPS 上。
